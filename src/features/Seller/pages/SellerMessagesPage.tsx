...
interface SellerMessageRouteState {
...
const getParticipant = (conversation: ChatConversation) =>
  conversation.participant ?? conversation.buyer ?? conversation.receiver ?? conversation.user
...
const normalizeConversation = (conversation: ChatConversation): NormalizedConversation | null => {
  ...
  const receiverId = conversation.receiver_id ?? conversation.buyer_id ?? conversation.user_id ?? participant?.id
  ...
  return {
    ...
    name: participant?.name ?? `Buyer #${receiverId}`,
...
const SellerMessagesPage = () => {
  ...
  const routeState = location.state as SellerMessageRouteState | null
  ...
  const conversations = useMemo(() => {
    ...
    const routeConversation: NormalizedConversation = {
      ...
      name: routeState.buyerName ?? `Buyer #${routeReceiverId}`,
      serviceTitle: routeState.serviceTitle ?? "Direct message",
      preview: "Start a conversation with this buyer.",
      time: "Now",
      unread: false,
    }
  ...
  const handleSendMessage = () => {
    ...
    if (!selectedReceiverId) {
      toast.error("Please select a buyer to message")
      return
    }
    ...
    <div className="grid min-h-[calc(100vh-120px)] gap-4 lg:grid-cols-[340px_1fr]">
      <aside className="rounded-2xl border border-border bg-card p-3">
        <div className="px-2 py-3">
          ...
          <p className="text-sm text-muted-foreground">Talk with buyers before and after booking.</p>
        </div>
        <div className="space-y-2">
          {isLoadingConversations && conversations.length === 0 ? (
            ...
            conversations.map((conversation) => (
              ...
              >
                <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-cyan-500/10 text-sm font-bold text-cyan-700 dark:text-cyan-300">
                ...
                {conversation.unread && <span className="mt-2 size-2 rounded-full bg-cyan-500" />}
              </button>
            ))
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-3 border-b border-border p-4">
          {activeConversation ? (
            <div className="flex items-center gap-3">
              <div className="flex size-11 items-center justify-center rounded-full bg-cyan-500/10 text-sm font-bold text-cyan-700 dark:text-cyan-300">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Choose a buyer to start chatting.</p>
              </div>
            </div>
          )}
              ...
              return (
                <div
                  ...
                  className={`max-w-[78%] rounded-2xl p-4 ${
                    isOutgoing
                      ? "ml-auto rounded-tr-sm bg-cyan-600 text-primary-foreground"
