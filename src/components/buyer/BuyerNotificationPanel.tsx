...
import { Bell, CheckCircle, CircleNotch, ShoppingBagOpen, X } from "@phosphor-icons/react"
...
export type BuyerNotificationsResponse =
  | BuyerNotificationItem[]
  | {
      notification?: BuyerNotificationItem[]
      data?: BuyerNotificationItem[]
    }

type BuyerNotificationPanelProps = {
  open: boolean
  onClose: () => void
  triggerRef: React.RefObject<HTMLButtonElement | null>
}

export const getBuyerNotifications = (data?: BuyerNotificationsResponse) => {
  return Array.isArray(data) ? data : data?.notification ?? data?.data ?? []
}

const formatNotificationTime = (notification: BuyerNotificationItem) => {
...
const getNotificationMessage = (notification: BuyerNotificationItem) => {
...
const BuyerNotificationPanel = ({ open, onClose, triggerRef }: BuyerNotificationPanelProps) => {
  const panelRef = useRef<HTMLDivElement>(null)
  const navigate = useNavigate()
  const { data, isLoading, isError } = useApiQuery<unknown, BuyerNotificationsResponse>({
    endpoint: "/notifications",
    queryKey: ["notifications"],
    enabled: open,
    raw: true,
  })
  const notifications = getBuyerNotifications(data)
  ...
  return (
    <>
      <button
        type="button"
        aria-label="Close buyer notifications"
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isError && !isLoading && (
            <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">Could not fetch buyer notifications.</p>
            </div>
          )}

          {!isLoading && !isError && notifications.length === 0 && (
            <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
              <CheckCircle size={42} weight="thin" className="mb-3 text-emerald-400/80" />
              <p className="text-sm font-semibold text-foreground">All caught up</p>
              <p className="mt-1 text-xs text-muted-foreground">You have no new buyer notifications.</p>
            </div>
          )}
          ...
          {!isLoading && !isError && notifications.length > 0 && (
            <div className="divide-y divide-border">
              {notifications.map((notification) => (
                ...
                >
                  <div className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-300">
